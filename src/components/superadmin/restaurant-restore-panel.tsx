...
export function RestaurantRestorePanel({restaurantId,restaurantSlug,backups}:{restaurantId:string;restaurantSlug:string;backups:StoredBackup[]}){
  ...
  return <section className="rounded-3xl border border-amber-400/20 bg-amber-400/[.04] p-5">
    <div className="flex items-center gap-2"><ShieldCheck size={18} className="text-amber-800"/><h2 className="font-bold">Restaurar copia</h2></div>
    <p className="mt-2 text-xs leading-relaxed text-slate-600">Primero se genera una vista previa. Ningún dato cambia hasta confirmar con el slug del restaurante.</p>
    ...
    <details className="mt-3 rounded-xl border border-stone-200 bg-white p-3"><summary className="flex cursor-pointer list-none items-center justify-between text-sm font-semibold"><span className="inline-flex items-center gap-2"><History size={16}/>Historial privado</span><span className="text-xs text-slate-500">{backups.length}</span></summary><div className="mt-3 space-y-2">{backups.map(item=><div key={item.id} className="rounded-lg border border-stone-200 p-2"><div className="flex items-start justify-between gap-2"><button type="button" onClick={()=>loadStoredBackup(item.id)} disabled={busy} className="min-w-0 text-left"><span className="block text-xs font-bold text-amber-800">{reasonLabel[item.reason]}</span><span className="block text-[11px] text-slate-500">{new Intl.DateTimeFormat("es-ES",{dateStyle:"short",timeStyle:"short"}).format(new Date(item.created_at))} · {item.product_count} productos</span></button><div className="flex shrink-0 gap-1"><a href={`/api/superadmin/restaurants/${restaurantId}/backups/${item.id}?download=1`} title="Descargar copia" className="rounded-md p-2 hover:bg-stone-100"><Download size={14}/></a><button type="button" onClick={()=>deleteBackup(item.id)} title={pendingDelete===item.id?"Confirmar eliminación":"Eliminar copia"} className={`rounded-md p-2 ${pendingDelete===item.id?"bg-red-600 text-white":"hover:bg-red-500/10 hover:text-red-700"}`}><Trash2 size={14}/></button></div></div>{pendingDelete===item.id&&<p className="mt-1 text-[10px] text-red-700">Pulsa otra vez para eliminarla.</p>}</div>)}{!backups.length&&<p className="text-xs text-slate-500">Todavía no hay copias guardadas.</p>}</div></details>
    ...
    {error&&<p role="alert" className="mt-3 rounded-xl bg-red-500/10 p-3 text-xs text-red-700">{error}</p>}
    {restored&&<p className="mt-3 rounded-xl bg-emerald-500/10 p-3 text-xs text-emerald-800">Copia restaurada correctamente.</p>}
    {preview&&<div className="mt-4 space-y-4">
      <div className="rounded-xl border border-stone-200 bg-white p-3"><p className="text-sm font-bold">{preview.sourceName}</p><p className="mt-1 text-xs text-slate-500">Copia del {new Intl.DateTimeFormat("es-ES",{dateStyle:"medium",timeStyle:"short"}).format(new Date(preview.exportedAt))}</p></div>
      ...
      <p className="text-xs text-slate-600">{preview.changedSettings.length} ajustes cambiarán · {preview.mediaReferences} referencias de medios</p>
      <ul className="space-y-1 text-xs text-amber-800">{preview.warnings.map(warning=><li key={warning}>• {warning}</li>)}</ul>
      {preview.canApply&&<><label className="block text-xs font-medium text-slate-700">Escribe <strong>{restaurantSlug}</strong> para confirmar<input value={confirmation} onChange={event=>setConfirmation(event.target.value)} autoComplete="off" className="mt-2 w-full rounded-xl border border-stone-200 bg-stone-100 p-3 text-sm"/></label><button type="button" onClick={applyRestore} disabled={busy||confirmation!==restaurantSlug} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-amber-600 px-4 py-3 text-sm font-bold disabled:cursor-not-allowed disabled:opacity-40"><RefreshCcw size={17}/>{busy?"Restaurando…":"Aplicar restauración"}</button></>}
    </div>}
  </section>;
}

function Difference({label,value}:{label:string;value:Preview["categories"]}){return <div className="rounded-xl bg-white p-3"><p className="text-xs text-slate-500">{label}</p><p className="mt-1 text-lg font-bold">{value.current} → {value.backup}</p><p className="text-[11px] text-slate-500">+{value.added} · −{value.removed}</p></div>}
