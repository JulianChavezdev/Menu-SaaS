...
export function DeleteRestaurantPanel({restaurantId,restaurantName,restaurantSlug,superadminEmail}:{restaurantId:string;restaurantName:string;restaurantSlug:string;superadminEmail:string}){
  ...
  return <details className="mt-6 rounded-3xl border border-red-500/25 bg-red-500/[.05] p-5">
    <summary className="flex cursor-pointer list-none items-center gap-2 font-bold text-red-300"><TriangleAlert size={19}/>Zona de peligro: eliminar restaurante</summary>
    <div className="mt-4 border-t border-red-500/15 pt-4"><p className="text-sm leading-relaxed text-slate-300">Eliminarás permanentemente <strong>{restaurantName}</strong>, su carta, miembros, pagos y analíticas. El usuario propietario no se elimina. Antes de continuar se guardará una copia administrativa final de la configuración y la carta.</p>
      <form action={deleteRestaurant} className="mt-4 grid gap-3">
        <input type="hidden" name="restaurant_id" value={restaurantId}/>
        <label className="text-sm text-slate-300">Confirma tu correo de superadmin<input required name="security_email" type="email" autoComplete="off" value={email} onChange={event=>setEmail(event.target.value)} placeholder="tu-correo@ejemplo.com" className="mt-1.5 w-full rounded-xl border border-red-500/20 bg-slate-950 p-3 text-white outline-none focus:border-red-400"/></label>
        <label className="text-sm text-slate-300">Escribe exactamente <code className="rounded bg-black/30 px-1.5 py-1 text-red-200">{phrase}</code><input required name="confirmation" autoComplete="off" spellCheck={false} value={confirmation} onChange={event=>setConfirmation(event.target.value)} className="mt-1.5 w-full rounded-xl border border-red-500/20 bg-slate-950 p-3 text-white outline-none focus:border-red-400"/></label>
        <label className="flex items-start gap-2 rounded-xl border border-red-500/15 p-3 text-xs leading-relaxed text-slate-400"><input required name="acknowledge" value="yes" type="checkbox" checked={acknowledged} onChange={event=>setAcknowledged(event.target.checked)} className="mt-0.5"/>Entiendo que esta acción no se puede deshacer desde el panel.</label>
