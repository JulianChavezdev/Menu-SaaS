...
export function ProductCustomizer({product,currency,language="es",panel,accent,onAccent,onClose,onConfirm,initial=[],appearance="dark"}:{product:Product;currency:string;language?:"es"|"en";panel:string;accent:string;onAccent:string;onClose:()=>void;onConfirm:(selection:CustomizationSelection)=>void;initial?:CustomizationSelection;appearance?:"dark"|"marshmallow"}){
  ...
  return <dialog ref={dialog} className="product-customizer" data-appearance={appearance} data-menu-gesture-block style={{background:panel,color:appearance==="marshmallow"?"#493332":"#fff","--custom-accent":accent} as React.CSSProperties} onPointerDown={e=>e.stopPropagation()} onPointerUp={e=>e.stopPropagation()} onTouchStart={e=>e.stopPropagation()} onTouchEnd={e=>e.stopPropagation()} onWheel={e=>e.stopPropagation()} onCancel={onClose} aria-label={`${es?"Personalizar":"Customize"} ${product.name}`}>
