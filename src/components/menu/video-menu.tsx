  ...
  return <main ref={feedRef} onTouchEnd={resumeActiveVideo} aria-label={`Carta de ${restaurant.name}`} data-template={template.key} data-hydrated={hydrated?"true":"false"} style={themeStyle} className="public-menu relative h-screen h-dvh snap-y snap-mandatory overflow-y-auto overscroll-y-contain scroll-smooth bg-[var(--theme-bg)] text-white md:mx-auto md:max-w-[402px]">
    ...
    <div>{products.map((product,index)=>{const description=translatedField(product,"description",language,product.description);const allergens=(product.allergens??[]) as AllergenCode[];return <section ref={element=>{sectionRefs.current[index]=element}} data-index={index} id={`product-${product.id}`} key={product.id} className="relative isolate flex h-screen h-dvh snap-start snap-always items-end overflow-hidden bg-[var(--theme-bg)] px-4 pb-[calc(5.5rem+max(.75rem,env(safe-area-inset-bottom)))] pt-24">
      ...
      <div data-product-details className="relative z-10 w-full max-h-[calc(100dvh-11rem-env(safe-area-inset-top)-env(safe-area-inset-bottom))] overflow-y-auto overscroll-contain pb-0.5 text-shadow-lg [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        <div className="mb-0.5 flex min-w-0 items-center gap-2"><span style={{color:colors.accent}} className="max-w-[65%] truncate text-[8px] font-bold uppercase tracking-[.12em]">{translatedField(product.categories??{},"name",language,product.categories?.name)}</span>{product.is_featured&&<span className="truncate text-[8px] font-medium text-white/65">★ {text.featured}</span>}</div>
        <div className="flex items-end justify-between gap-3"><div className="min-w-0"><h2 className="line-clamp-1 text-[clamp(1.15rem,5.3vw,1.5rem)] font-semibold leading-tight tracking-[-.02em]">{translatedField(product,"name",language,product.name)}</h2><strong style={{color:colors.accent}} className="mt-1 block text-base font-semibold tabular-nums">{currency.format(product.price_cents/100)}</strong></div><button onClick={()=>addProduct(product.id)} style={{background:colors.accent,color:colors.background}} className="flex min-h-9 shrink-0 items-center gap-1 rounded-full px-3 text-[11px] font-bold shadow-lg transition active:scale-95"><Plus size={14}/>{text.add}</button></div>
        {(description||allergens.length>0)&&<details onToggle={event=>revealExpandedDetails(event.currentTarget)} className="group mt-0.5"><summary className="flex cursor-pointer list-none items-center gap-1 text-[10px] font-semibold text-white/75">{text.description}<ChevronDown size={12} className="transition-transform group-open:rotate-180"/></summary>{description&&<p className="mt-1 text-[11px] leading-[1.3] text-white/70">{description}</p>}{allergens.length>0&&<div className="mt-1.5 border-t border-white/10 pt-1"><p className="flex items-center gap-1 text-[9px] font-semibold text-white/65"><TriangleAlert size={10}/>{text.allergens} · {allergens.length}</p><div className="mt-1 flex flex-wrap gap-1">{allergens.map(code=><span key={code} style={{borderColor:colors.frame,background:`${colors.panel}d9`}} className="rounded-full border px-2 py-0.5 text-[9px] font-semibold">{allergenLabel(code,language)}</span>)}</div><p className="mt-1 text-[9px] leading-snug text-white/50">{text.allergenNotice}</p></div>}</details>}
      </div>
    </section>})}</div>
