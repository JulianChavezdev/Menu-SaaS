...
export function VideoMenu({restaurant,products,analyticsEnabled=true}:{restaurant:Restaurant;products:Product[];analyticsEnabled?:boolean}){
  ...
  useEffect(()=>{
    const sectionObserver=new IntersectionObserver(entries=>entries.forEach(entry=>{if(entry.isIntersecting&&entry.intersectionRatio>.6)setActive(Number((entry.target as HTMLElement).dataset.index??0))}),{root:feedRef.current,threshold:[.6]});
    ...
    const videoObserver=new IntersectionObserver(entries=>entries.forEach(entry=>{
      ...
      if(entry.isIntersecting&&entry.intersectionRatio>.7){
        ...
        void video.play().then(()=>{trackVideoPlay(index);setPlaybackBlocked(current=>{if(!current.has(index))return current;const next=new Set(current);next.delete(index);return next})}).catch(()=>{if(playingIndex.current===index){playingIndex.current=null;setPlaybackBlocked(current=>new Set(current).add(index))}});
  ...
  },[active,products,trackVideoPlay]);
  ...
  const resumeActiveVideo=useCallback(()=>{const feed=feedRef.current;const visibleIndex=feed?Math.max(0,Math.min(products.length-1,Math.round(feed.scrollTop/feed.clientHeight))):active;const video=videoRefs.current[visibleIndex];if(!video||reducedMotion||!products[visibleIndex]?.video_url)return;video.muted=muted;void video.play().then(()=>playbackStarted(visibleIndex)).catch(()=>setPlaybackBlocked(current=>new Set(current).add(visibleIndex)))},[active,muted,playbackStarted,products,reducedMotion]);
  useEffect(()=>{const resume=()=>{if(document.visibilityState==="visible")resumeActiveVideo()};document.addEventListener("visibilitychange",resume);addEventListener("pageshow",resume);return()=>{document.removeEventListener("visibilitychange",resume);removeEventListener("pageshow",resume)}},[resumeActiveVideo]);
  ...
  return <main ref={feedRef} onTouchEnd={resumeActiveVideo} aria-label={`Carta de ${restaurant.name}`} data-template={template.key} data-hydrated={hydrated?"true":"false"} style={themeStyle} className="public-menu relative h-screen h-dvh snap-y snap-mandatory overflow-y-auto overscroll-y-contain scroll-smooth bg-[var(--theme-bg)] text-white md:mx-auto md:max-w-[430px] md:border-x md:border-white/10 md:shadow-2xl">
    ...
    {panel&&<div className="fixed inset-0 z-50 mx-auto flex max-w-[430px] items-end bg-black/65 p-3 backdrop-blur-sm" onClick={()=>setPanel(null)}>
      <aside aria-label={panel==="menu"?text.menu:panel==="cart"?text.cart:text.info} style={{background:colors.panel,borderColor:colors.frame}} className="flex max-h-[88vh] max-h-[88dvh] w-full flex-col rounded-xl border p-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] shadow-2xl" onClick={event=>event.stopPropagation()}>
        ...
        {panel==="menu"?<div className="mt-4 min-h-0 overflow-y-auto pr-1"><p className="mb-5 text-xs text-white/55">{text.listHint}</p><div className="space-y-7">{categoryGroups.map(group=><section key={group.id}><h3 style={{color:colors.accent}} className="mb-3 text-xs font-bold uppercase tracking-[.16em]">{group.name}</h3><div className="grid grid-cols-2 gap-3">{group.products.map(product=><article key={product.id} className="min-w-0 overflow-hidden rounded-2xl border border-white/10 bg-white/[.04]"><button type="button" aria-label={`Ver ${product.name}`} onClick={()=>go(`product-${product.id}`)} className="relative block aspect-square w-full overflow-hidden bg-black/30">{product.image_url?<span role="img" aria-label={product.name} style={{backgroundImage:`url(${product.image_url})`}} className="block h-full w-full bg-cover bg-center"/>:<span className="grid h-full place-items-center bg-gradient-to-br from-white/10 to-black/30 text-white/45"><List size={24}/></span>}<span className="absolute inset-0 bg-gradient-to-t from-black/45 to-transparent"/></button><div className="p-3"><button type="button" onClick={()=>go(`product-${product.id}`)} className="line-clamp-2 min-h-9 text-left text-sm font-semibold leading-tight">{translatedField(product,"name",language,product.name)}</button><div className="mt-2 flex items-center justify-between gap-2"><strong style={{color:colors.accent}} className="text-sm tabular-nums">{currency.format(product.price_cents/100)}</strong><button type="button" aria-label={`${text.add} ${product.name}`} onClick={()=>addProduct(product.id)} style={{background:colors.accent,color:colors.background}} className="grid h-8 w-8 shrink-0 place-items-center rounded-full"><Plus size={15}/></button></div>{product.allergens?.length?<p className="mt-2 line-clamp-2 text-[9px] leading-tight text-white/45">{product.allergens.map(code=>allergenLabel(code,language)).join(" · ")}</p>:null}</div></article>)}</div></section>)}</div></div>
    ...
    <div>{products.map((product,index)=>{const description=translatedField(product,"description",language,product.description);const allergens=(product.allergens??[]) as AllergenCode[];return <section ref={element=>{sectionRefs.current[index]=element}} data-index={index} id={`product-${product.id}`} key={product.id} className="relative isolate flex h-screen h-dvh snap-start snap-always items-end overflow-hidden bg-[var(--theme-bg)] px-4 pb-[calc(8rem+max(.75rem,env(safe-area-inset-bottom)))] pt-28">
      <div style={{borderColor:colors.frame}} className={`absolute z-0 overflow-hidden bg-[#22221f] ${framed?"inset-3 bottom-24 rounded-xl border shadow-2xl":"inset-0"}`}><ProductMedia index={index} name={product.name} src={product.video_url} poster={product.image_url} muted={muted} preload={index>=active&&index<=active+1?"auto":"metadata"} active={index===active} hydrated={Math.abs(index-active)<=1} reducedMotion={reducedMotion} playbackBlocked={playbackBlocked.has(index)} setVideoRef={element=>{videoRefs.current[index]=element}} onPlaybackStarted={playbackStarted}/></div>
      <div className={`absolute z-[1] ${framed?"inset-3 bottom-24 rounded-xl":"inset-0"}`} style={{background:`linear-gradient(180deg,${colors.background}66 0%,transparent 32%,transparent 45%,${colors.background}f2 100%)`}}/>
      ...
      <div data-product-details className="relative z-10 w-full max-h-[calc(100vh-16rem)] max-h-[calc(100dvh-16rem-env(safe-area-inset-top)-env(safe-area-inset-bottom))] overflow-y-auto overscroll-contain pb-1 text-shadow-lg [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        {description&&<details onToggle={event=>revealExpandedDetails(event.currentTarget)} className="group mt-2 max-w-[350px]"><summary className="flex cursor-pointer list-none items-center gap-1.5 text-[12px] font-semibold text-white/80">{text.description}<ChevronDown size={15} className="transition-transform group-open:rotate-180"/></summary><p className="mt-1.5 text-[13px] leading-[1.45] text-white/75">{description}</p></details>}
        {allergens.length>0&&<details onToggle={event=>revealExpandedDetails(event.currentTarget)} className="group mt-2 max-w-[350px]"><summary className="flex cursor-pointer list-none items-center gap-1.5 text-[12px] font-semibold text-white/80"><TriangleAlert size={14}/>{text.allergens} · {allergens.length}<ChevronDown size={15} className="transition-transform group-open:rotate-180"/></summary><div className="mt-2 flex flex-wrap gap-1.5">{allergens.map(code=><span key={code} style={{borderColor:colors.frame,background:`${colors.panel}d9`}} className="rounded-full border px-2.5 py-1 text-[10px] font-semibold">{allergenLabel(code,language)}</span>)}</div><p className="mt-2 text-[10px] leading-relaxed text-white/55">{text.allergenNotice}</p></details>}
      </div>
    </section>})}</div>

    <nav ref={categoryNavRef} aria-label={text.categories} className="fixed left-1/2 top-[calc(max(1rem,env(safe-area-inset-top))+3.5rem)] z-40 flex w-[calc(100%-1.5rem)] max-w-[398px] -translate-x-1/2 snap-x snap-mandatory gap-2 overflow-x-auto [mask-image:linear-gradient(to_right,transparent,black_14%,black_86%,transparent)] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
