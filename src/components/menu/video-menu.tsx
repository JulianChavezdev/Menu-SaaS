...
const copy={
  ...
  en:{menu:"Menu",close:"Close",share:"Share",info:"Restaurant",soundOn:"Turn sound on",soundOff:"Mute",website:"Visit website",categories:"Categories",featured:"Featured",description:"Description",allergens:"Allergens",pairings:"Pairs well with",allergenNotice:"If you have an allergy, always confirm the information with staff.",listHint:"Tap a dish to view its video.",add:"Add",cart:"Cart",emptyCart:"Your cart is empty",total:"Total",note:"Notes",notePlaceholder:"Add or remove ingredients",remove:"Remove",saved:"Saved on this device. It is not sent to the kitchen."},
} as const;
...
export function VideoMenu({restaurant,products,analyticsEnabled=true,introEnabled=true,tableOrdering=null}:{restaurant:Restaurant;products:Product[];analyticsEnabled?:boolean;introEnabled?:boolean;tableOrdering?:TableOrderingContext|null}){
  ...
  const toggleDetails=(productId:string,details:HTMLDetailsElement)=>{setExpandedDetails(current=>{const next=new Set(current);if(details.open)next.add(productId);else next.delete(productId);return next});revealExpandedDetails(details);if(details.open&&analyticsEnabled&&!openedDetails.current.has(productId)){openedDetails.current.add(productId);sendAnalytics({restaurantId:restaurant.id,productId,event:"detail_open",locale:language})}};

  return <main ref={feedRef} onTouchEnd={resumeActiveVideo} onPointerUp={resumeActiveVideo} aria-label={`Carta de ${restaurant.name}`} data-template={template.key} data-hydrated={hydrated?"true":"false"} style={themeStyle} className="public-menu relative h-svh snap-y snap-mandatory overflow-y-auto overscroll-none scroll-smooth bg-[var(--theme-bg)] text-white [scrollbar-width:none] [&::-webkit-scrollbar]:hidden md:mx-auto md:max-w-[402px]">
    ...
    <div>{products.map((product,index)=>{const description=translatedField(product,"description",language,product.description);const allergens=(product.allergens??[]) as AllergenCode[];const recommendations=product.recommended_products?.filter(item=>item.is_available)??[];return <section ref={element=>{sectionRefs.current[index]=element}} data-index={index} id={`product-${product.id}`} key={product.id} className="relative isolate flex h-svh snap-start snap-always items-end overflow-hidden bg-[var(--theme-bg)] px-4 pb-[var(--controls-clearance)] pt-24">
      ...
      {template.key!=="cinematic"&&<div className={`absolute z-[1] ${framed?"inset-3 bottom-16 rounded-xl":"inset-0"}`} style={{background:`linear-gradient(180deg,${colors.background}66 0%,transparent 32%,transparent 45%,${colors.background}f2 100%)`}}/>}
      ...
      {primaryTemplate&&expandedDetails.has(product.id)&&<div data-description-backdrop aria-hidden="true" className="pointer-events-none absolute inset-x-0 bottom-0 z-[3] h-[58%] bg-gradient-to-t from-black/95 via-black/65 to-transparent"/>}
      <div data-product-details className="relative z-10 w-full max-h-[calc(100dvh-11rem-env(safe-area-inset-top)-env(safe-area-inset-bottom))] overflow-y-auto overscroll-contain pb-0.5 text-shadow-lg [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        <div className={`flex min-h-8 gap-2 ${primaryTemplate?"items-start":"items-center"}`}><h2 className={`min-w-0 flex-1 font-semibold leading-tight tracking-[-.015em] ${primaryTemplate?"line-clamp-2 text-[clamp(1.3rem,6vw,1.6rem)]":"line-clamp-1 text-[clamp(1.05rem,4.8vw,1.3rem)]"}`}>{product.is_featured&&<span style={{color:colors.accent}} className="mr-1 text-[9px] align-middle">★</span>}{translatedField(product,"name",language,product.name)}</h2><strong style={{color:colors.accent}} className="shrink-0 pt-0.5 text-[14px] font-semibold tabular-nums">{currency.format(product.price_cents/100)}</strong><button aria-label={`${text.add} ${product.name}`} title={text.add} onClick={()=>addProduct(product.id)} style={{background:colors.accent,color:colors.background}} className="grid h-8 w-8 shrink-0 place-items-center rounded-full shadow-lg transition active:scale-90"><Plus size={14}/></button></div>
        {(description||allergens.length>0||recommendations.length>0)&&<details onToggle={event=>toggleDetails(product.id,event.currentTarget)} className="group mt-1"><summary className={`flex cursor-pointer list-none items-center gap-1 font-semibold text-white/75 ${primaryTemplate?"text-[12px]":"text-[9px]"}`}>{text.description}<ChevronDown size={primaryTemplate?14:11} className="transition-transform group-open:rotate-180"/></summary>{description&&<p className={`mt-1 leading-snug text-white/75 ${primaryTemplate?"text-[12px]":"text-[10px]"}`}>{description}</p>}{allergens.length>0&&<div className="mt-1 border-t border-white/10 pt-1"><p className="flex items-center gap-1 text-[9px] font-semibold text-white/65"><TriangleAlert size={10}/>{text.allergens} · {allergens.length}</p><div className="mt-1 flex flex-wrap gap-1">{allergens.map(code=><span key={code} style={{borderColor:colors.frame,background:`${colors.panel}d9`}} className="rounded-full border px-2 py-0.5 text-[9px] font-semibold">{allergenLabel(code,language)}</span>)}</div><p className="mt-1 text-[9px] leading-snug text-white/50">{text.allergenNotice}</p></div>}{recommendations.length>0&&<div className="mt-1.5 border-t border-white/10 pt-1.5"><p className="text-[9px] font-semibold text-white/65">{text.pairings}</p><div className="mt-1 grid gap-1">{recommendations.map(item=>{const added=catalogAdded===item.id;return <div key={item.id} className="flex items-center gap-2 rounded-lg bg-black/20 p-1"><span role="img" aria-label="" style={item.image_url?{backgroundImage:`url(${item.image_url})`}:undefined} className="h-8 w-8 shrink-0 rounded-md bg-white/10 bg-cover bg-center"/><span className="min-w-0 flex-1 truncate text-[10px] font-semibold">{translatedField(item,"name",language,item.name)}</span><strong style={{color:colors.accent}} className="text-[9px] tabular-nums">{currency.format(item.price_cents/100)}</strong><button type="button" aria-label={`${text.add} ${item.name}`} onClick={()=>addRecommendation(item.id)} style={{background:colors.accent,color:colors.background}} className={`grid h-7 w-7 shrink-0 place-items-center rounded-full transition ${added?"scale-110":"active:scale-90"}`}>{added?<Check size={14}/>:<Plus size={13}/>}</button></div>})}</div></div>}</details>}
      </div>
    </section>})}</div>

    <nav ref={categoryNavRef} aria-label={text.categories} className="fixed left-1/2 top-[calc(max(1rem,env(safe-area-inset-top))+3.25rem)] z-40 flex w-[calc(100%-2rem)] max-w-[390px] -translate-x-1/2 touch-pan-x snap-x snap-mandatory gap-2 overflow-x-auto overscroll-x-contain [mask-image:linear-gradient(to_right,transparent,black_14%,black_86%,transparent)] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden md:max-w-[370px]">
