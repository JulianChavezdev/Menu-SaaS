...
export function VideoMenu({restaurant,products,analyticsEnabled=true,introEnabled=true}:{restaurant:Restaurant;products:Product[];analyticsEnabled?:boolean;introEnabled?:boolean}){
  ...
  return <main ref={feedRef} onTouchEnd={resumeActiveVideo} aria-label={`Carta de ${restaurant.name}`} data-template={template.key} data-hydrated={hydrated?"true":"false"} style={themeStyle} className="public-menu relative h-screen h-dvh snap-y snap-mandatory overflow-y-auto overscroll-y-contain scroll-smooth bg-[var(--theme-bg)] text-white md:mx-auto md:max-w-[430px] md:border-x md:border-white/10 md:shadow-2xl lg:max-w-none lg:border-x-0 lg:shadow-none">
    ...
    <div>{products.map((product,index)=>{const description=translatedField(product,"description",language,product.description);const allergens=(product.allergens??[]) as AllergenCode[];return <section ref={element=>{sectionRefs.current[index]=element}} data-index={index} id={`product-${product.id}`} key={product.id} className="relative isolate flex h-screen h-dvh snap-start snap-always items-end overflow-hidden bg-[var(--theme-bg)] px-4 pb-[calc(5.5rem+max(.75rem,env(safe-area-inset-bottom)))] pt-24 lg:items-center lg:justify-center lg:pb-16 lg:pl-[440px] lg:pr-24 lg:pt-24">
      ...
      <div data-product-details style={{borderColor:colors.frame,background:`${colors.panel}e8`}} className="relative z-10 w-full max-h-[calc(100dvh-11rem-env(safe-area-inset-top)-env(safe-area-inset-bottom))] overflow-y-auto overscroll-contain pb-0.5 text-shadow-lg [scrollbar-width:none] [&::-webkit-scrollbar]:hidden lg:max-h-[calc(100dvh-10rem)] lg:max-w-[430px] lg:rounded-xl lg:border lg:p-6 lg:text-shadow-none lg:shadow-2xl lg:backdrop-blur-xl">
        ...
        <h2 className="line-clamp-1 max-w-[340px] text-[clamp(1.3rem,6vw,1.75rem)] font-semibold leading-tight tracking-[-.025em]">{translatedField(product,"name",language,product.name)}</h2>
        <div className="mt-1.5 flex items-center justify-between gap-3"><strong style={{color:colors.accent}} className="text-lg font-semibold tabular-nums">{currency.format(product.price_cents/100)}</strong><button onClick={()=>addProduct(product.id)} style={{background:colors.accent,color:colors.background}} className="flex min-h-9 items-center gap-1.5 rounded-full px-3 text-xs font-bold shadow-lg transition active:scale-95"><Plus size={15}/>{text.add}</button></div>
        {(description||allergens.length>0)&&<details onToggle={event=>revealExpandedDetails(event.currentTarget)} className="group mt-1"><summary className="flex cursor-pointer list-none items-center gap-1 text-[11px] font-semibold text-white/75">{text.description}<ChevronDown size={13} className="transition-transform group-open:rotate-180"/></summary>{description&&<p className="mt-1 text-xs leading-[1.35] text-white/70">{description}</p>}{allergens.length>0&&<div className="mt-2 border-t border-white/10 pt-1.5"><p className="flex items-center gap-1 text-[10px] font-semibold text-white/65"><TriangleAlert size={11}/>{text.allergens} · {allergens.length}</p><div className="mt-1 flex flex-wrap gap-1">{allergens.map(code=><span key={code} style={{borderColor:colors.frame,background:`${colors.panel}d9`}} className="rounded-full border px-2 py-0.5 text-[9px] font-semibold">{allergenLabel(code,language)}</span>)}</div><p className="mt-1 text-[9px] leading-snug text-white/50">{text.allergenNotice}</p></div>}</details>}
      </div>
    </section>})}</div>
