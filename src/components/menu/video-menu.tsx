...
import {ArrowLeft,ChevronDown,Info,Languages,List,MapPin,Minus,Phone,Plus,Share2,ShoppingBag,Trash2,TriangleAlert,Volume2,VolumeX,X} from "lucide-react";
...
export function VideoMenu({restaurant,products,analyticsEnabled=true}:{restaurant:Restaurant;products:Product[];analyticsEnabled?:boolean}){
  ...
  const resumeActiveVideo=useCallback(()=>{const feed=feedRef.current;const visibleIndex=feed?Math.max(0,Math.min(products.length-1,Math.round(feed.scrollTop/feed.clientHeight))):active;const video=videoRefs.current[visibleIndex];if(!video||reducedMotion||!products[visibleIndex]?.video_url)return;if(video.error||video.networkState===HTMLMediaElement.NETWORK_NO_SOURCE)video.load();startVideo(video,visibleIndex)},[active,products,reducedMotion,startVideo]);
  ...
  return <main ref={feedRef} onTouchEnd={resumeActiveVideo} aria-label={`Carta de ${restaurant.name}`} data-template={template.key} data-hydrated={hydrated?"true":"false"} style={themeStyle} className="public-menu relative h-screen h-dvh snap-y snap-mandatory overflow-y-auto overscroll-y-contain scroll-smooth bg-[var(--theme-bg)] text-white md:mx-auto md:max-w-[430px] md:border-x md:border-white/10 md:shadow-2xl">
    ...
    {panel&&<div className="fixed inset-0 z-50 mx-auto flex max-w-[430px] items-end bg-black/65 p-3 backdrop-blur-sm" onClick={()=>setPanel(null)}>
      <aside aria-label={panel==="menu"?text.menu:panel==="cart"?text.cart:text.info} style={{background:colors.panel,borderColor:colors.frame}} className="flex max-h-[88vh] max-h-[88dvh] w-full flex-col rounded-xl border p-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] shadow-2xl" onClick={event=>event.stopPropagation()}>
        ...
        {panel==="menu"?<div className="mt-3 min-h-0 overflow-y-auto pr-1"><p className="mb-3 text-[11px] text-white/55">{text.listHint}</p><div className="space-y-4">{categoryGroups.map(group=><section key={group.id}><h3 style={{color:colors.accent}} className="mb-2 text-[10px] font-bold uppercase tracking-[.14em]">{group.name}</h3><div className="grid grid-cols-2 gap-2">{group.products.map(product=><article key={product.id} className="min-w-0 overflow-hidden rounded-xl border border-white/10 bg-white/[.04]"><button type="button" aria-label={`Ver ${product.name}`} onClick={()=>go(`product-${product.id}`)} className="relative block aspect-[4/3] w-full overflow-hidden bg-black/30">{product.image_url?<span role="img" aria-label={product.name} style={{backgroundImage:`url(${product.image_url})`}} className="block h-full w-full bg-cover bg-center"/>:<span className="grid h-full place-items-center bg-gradient-to-br from-white/10 to-black/30 text-white/45"><List size={20}/></span>}<span className="absolute inset-0 bg-gradient-to-t from-black/45 to-transparent"/></button><div className="p-2"><button type="button" onClick={()=>go(`product-${product.id}`)} className="line-clamp-1 w-full text-left text-xs font-semibold leading-tight">{translatedField(product,"name",language,product.name)}</button><div className="mt-1.5 flex items-center justify-between gap-2"><strong style={{color:colors.accent}} className="text-xs tabular-nums">{currency.format(product.price_cents/100)}</strong><button type="button" aria-label={`${text.add} ${product.name}`} onClick={()=>addProduct(product.id)} style={{background:colors.accent,color:colors.background}} className="grid h-7 w-7 shrink-0 place-items-center rounded-full"><Plus size={14}/></button></div></div></article>)}</div></section>)}</div></div>
    ...
    <div>{products.map((product,index)=>{const description=translatedField(product,"description",language,product.description);const allergens=(product.allergens??[]) as AllergenCode[];return <section ref={element=>{sectionRefs.current[index]=element}} data-index={index} id={`product-${product.id}`} key={product.id} className="relative isolate flex h-screen h-dvh snap-start snap-always items-end overflow-hidden bg-[var(--theme-bg)] px-4 pb-[calc(4.5rem+max(.5rem,env(safe-area-inset-bottom)))] pt-24">
      ...
      <div data-product-details className="relative z-10 w-full max-h-[calc(100dvh-11rem-env(safe-area-inset-top)-env(safe-area-inset-bottom))] overflow-y-auto overscroll-contain pb-0.5 text-shadow-lg [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        <div className="mt-1 flex flex-wrap gap-x-4 gap-y-1">
          {description&&<details onToggle={event=>revealExpandedDetails(event.currentTarget)} className="group open:basis-full"><summary className="flex cursor-pointer list-none items-center gap-1 text-[11px] font-semibold text-white/75">{text.description}<ChevronDown size={13} className="transition-transform group-open:rotate-180"/></summary><p className="mt-1 text-xs leading-[1.35] text-white/70">{description}</p></details>}
          {allergens.length>0&&<details onToggle={event=>revealExpandedDetails(event.currentTarget)} className="group open:basis-full"><summary className="flex cursor-pointer list-none items-center gap-1 text-[11px] font-semibold text-white/75"><TriangleAlert size={12}/>{text.allergens} · {allergens.length}<ChevronDown size={13} className="transition-transform group-open:rotate-180"/></summary><div className="mt-1 flex flex-wrap gap-1">{allergens.map(code=><span key={code} style={{borderColor:colors.frame,background:`${colors.panel}d9`}} className="rounded-full border px-2 py-0.5 text-[9px] font-semibold">{allergenLabel(code,language)}</span>)}</div><p className="mt-1 text-[9px] leading-snug text-white/50">{text.allergenNotice}</p></details>}
        </div>
      </div>
    </section>})}</div>
    ...
    <nav aria-label="Controles de la carta" style={{background:`${colors.nav}ed`,borderColor:colors.frame}} className="fixed bottom-[max(.5rem,env(safe-area-inset-bottom))] left-1/2 z-40 grid w-[calc(100%-2rem)] max-w-[390px] -translate-x-1/2 grid-cols-5 items-center rounded-xl border px-1 py-1 shadow-2xl backdrop-blur-xl">
      <button aria-label={text.menu} onClick={()=>setPanel("menu")} className="flex min-w-0 flex-col items-center gap-0.5 rounded-lg py-1 text-[8px] text-white/80"><List size={18}/>{text.menu}</button>
      <button aria-label={muted?text.soundOn:text.soundOff} onClick={()=>setMuted(value=>!value)} className="flex min-w-0 flex-col items-center gap-0.5 rounded-lg py-1 text-[8px] text-white/80">{muted?<VolumeX size={18}/>:<Volume2 size={18}/>}Audio</button>
      <button aria-label={`${text.cart}: ${cartQuantity}`} onClick={()=>setPanel("cart")} className="relative flex min-w-0 flex-col items-center gap-0.5 rounded-lg py-1 text-[8px] font-semibold text-white"><ShoppingBag size={19}/>{text.cart}{cartQuantity>0&&<span style={{background:colors.accent,color:colors.background}} className="absolute right-[20%] -top-0.5 grid h-4 min-w-4 place-items-center rounded-full px-1 text-[9px] font-black">{cartQuantity}</span>}</button>
      <button aria-label={text.info} onClick={()=>setPanel("info")} className="flex min-w-0 flex-col items-center gap-0.5 rounded-lg py-1 text-[8px] text-white/80"><Info size={18}/>{text.info}</button>
      <button aria-label={text.share} onClick={share} className="flex min-w-0 flex-col items-center gap-0.5 rounded-lg py-1 text-[8px] text-white/80"><Share2 size={18}/>{text.share}</button>
    </nav>
  </main>;
}
