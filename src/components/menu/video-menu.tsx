...
export function VideoMenu({restaurant,products}:{restaurant:Restaurant;products:Product[]}){
  ...
  return <main data-template={template.key} className={`public-menu relative h-dvh snap-y snap-mandatory overflow-y-auto overscroll-y-contain scroll-smooth text-white md:mx-auto md:max-w-[430px] md:border-x md:border-white/10 md:shadow-2xl ${midnight?"bg-[#050817]":"bg-[#0b0b0a]"}`}>
    <header className={`pointer-events-none fixed left-0 right-0 top-0 z-30 mx-auto flex max-w-[430px] items-center justify-between bg-gradient-to-b px-4 pb-10 pt-[max(1rem,env(safe-area-inset-top))] ${midnight?"from-[#050817] via-[#050817]/70 to-transparent":"from-black/85 via-black/45 to-transparent"}`}>
    ...
    <div>
      {products.map((product,index)=><section ref={element=>{sectionRefs.current[index]=element}} data-index={index} id={`product-${product.id}`} key={product.id} className={`relative isolate flex h-dvh snap-start snap-always items-end overflow-hidden px-5 pb-32 pt-28 ${midnight?"bg-[#050817]":""}`}>
        <div className={`absolute -z-20 overflow-hidden bg-[#22221f] ${midnight?"inset-3 bottom-24 rounded-[32px] border border-cyan-200/15 shadow-[0_0_60px_rgba(34,211,238,.12)]":"inset-0"}`}>{product.video_url?<video ref={element=>{videoRefs.current[index]=element}} src={product.video_url} poster={product.image_url??undefined} muted={muted} loop playsInline preload={index===0?"metadata":"none"} className="h-full w-full object-cover"/>:<div className="h-full w-full bg-cover bg-center" style={{backgroundImage:product.image_url?`url(${product.image_url})`:undefined}}/>}</div>
    ...
    <nav aria-label="Controles de la carta" className={`fixed bottom-[max(.75rem,env(safe-area-inset-bottom))] left-1/2 z-40 flex w-[calc(100%-1.5rem)] max-w-[398px] -translate-x-1/2 items-center justify-around rounded-2xl border border-white/15 px-2 py-2 shadow-2xl backdrop-blur-xl ${midnight?"bg-[#090e27]/90":"bg-[#171715]/90"}`}>
