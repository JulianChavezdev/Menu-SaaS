...
export function VideoMenu({restaurant,products,analyticsEnabled=true}:{restaurant:Restaurant;products:Product[];analyticsEnabled?:boolean}){
  ...
  const categoryGroups=[...products.reduce((groups,product)=>{const name=translatedField(product.categories??{},"name",language,product.categories?.name??text.menu);const current=groups.get(product.category_id);if(current)current.products.push(product);else groups.set(product.category_id,{name,products:[product]});return groups},new Map<string,{name:string;products:Product[]}>()).values()];
  ...
  return <main aria-label={`Carta de ${restaurant.name}`} data-template={template.key} style={themeStyle} className="public-menu relative h-screen h-dvh snap-y snap-mandatory overflow-y-auto overscroll-y-contain scroll-smooth bg-[var(--theme-bg)] text-white md:mx-auto md:max-w-[430px] md:border-x md:border-white/10 md:shadow-2xl">
    ...
    {panel&&<div className="fixed inset-0 z-50 mx-auto flex max-w-[430px] items-end bg-black/65 p-3 backdrop-blur-sm" onClick={()=>setPanel(null)}>
      <aside aria-label={panel==="menu"?text.menu:panel==="cart"?text.cart:text.info} style={{background:colors.panel,borderColor:colors.frame}} className="flex max-h-[88vh] max-h-[88dvh] w-full flex-col rounded-[28px] border p-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] shadow-2xl" onClick={event=>event.stopPropagation()}>
        ...
        {panel==="menu"?<div className="mt-4 min-h-0 overflow-y-auto pr-1"><p className="mb-5 text-xs text-white/55">{text.listHint}</p><div className="space-y-7">{categoryGroups.map(group=><section key={group.name}><h3 style={{color:colors.accent}} className="mb-3 text-xs font-bold uppercase tracking-[.16em]">{group.name}</h3><div className="grid grid-cols-2 gap-3">{group.products.map(product=><article key={product.id} className="min-w-0 overflow-hidden rounded-2xl border border-white/10 bg-white/[.04]"><button type="button" aria-label={`Ver ${product.name}`} onClick={()=>go(`product-${product.id}`)} className="relative block aspect-square w-full overflow-hidden bg-black/30">{product.video_url?<video src={`${product.video_url}#t=0.1`} poster={product.image_url??undefined} muted playsInline preload="metadata" className="h-full w-full object-cover"/>:product.image_url?<span role="img" aria-label={product.name} style={{backgroundImage:`url(${product.image_url})`}} className="block h-full w-full bg-cover bg-center"/>:<span className="grid h-full place-items-center text-white/35"><List size={24}/></span>}<span className="absolute inset-0 bg-gradient-to-t from-black/45 to-transparent"/></button><div className="p-3"><button type="button" onClick={()=>go(`product-${product.id}`)} className="line-clamp-2 min-h-9 text-left text-sm font-semibold leading-tight">{translatedField(product,"name",language,product.name)}</button><div className="mt-2 flex items-center justify-between gap-2"><strong style={{color:colors.accent}} className="text-sm tabular-nums">{currency.format(product.price_cents/100)}</strong><button type="button" aria-label={`${text.add} ${product.name}`} onClick={()=>addProduct(product.id)} style={{background:colors.accent,color:colors.background}} className="grid h-8 w-8 shrink-0 place-items-center rounded-full"><Plus size={15}/></button></div>{product.allergens?.length?<p className="mt-2 line-clamp-2 text-[9px] leading-tight text-white/45">{product.allergens.map(code=>allergenLabel(code,language)).join(" · ")}</p>:null}</div></article>)}</div></section>)}</div></div>
    ...
    <div>{products.map((product,index)=>{const description=translatedField(product,"description",language,product.description);const allergens=(product.allergens??[]) as AllergenCode[];return <section ref={element=>{sectionRefs.current[index]=element}} data-index={index} id={`product-${product.id}`} key={product.id} className="relative isolate flex h-screen h-dvh snap-start snap-always items-end overflow-hidden bg-[var(--theme-bg)] px-4 pb-32 pt-28">
      <div style={{borderColor:colors.frame}} className={`absolute z-0 overflow-hidden bg-[#22221f] ${framed?"inset-3 bottom-24 rounded-[32px] border shadow-2xl":"inset-0"}`}><ProductMedia index={index} name={product.name} src={product.video_url} poster={product.image_url} muted={muted} preload={index===active?"auto":index<=active+1?"metadata":"none"} active={index===active} reducedMotion={reducedMotion} playbackBlocked={playbackBlocked.has(index)} setVideoRef={element=>{videoRefs.current[index]=element}} onPlaybackStarted={playbackStarted}/></div>
