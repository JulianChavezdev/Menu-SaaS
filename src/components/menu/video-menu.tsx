...
export function VideoMenu({restaurant,products,analyticsEnabled=true,introEnabled=true}:{restaurant:Restaurant;products:Product[];analyticsEnabled?:boolean;introEnabled?:boolean}){
  ...
  return <main ref={feedRef} onTouchEnd={resumeActiveVideo} aria-label={`Carta de ${restaurant.name}`} data-template={template.key} data-hydrated={hydrated?"true":"false"} style={themeStyle} className="public-menu relative h-screen h-dvh snap-y snap-mandatory overflow-y-auto overscroll-y-contain scroll-smooth bg-[var(--theme-bg)] text-white md:mx-auto md:max-w-[430px] md:border-x md:border-white/10 md:shadow-2xl lg:max-w-none lg:border-x-0 lg:shadow-none">
    <h1 className="sr-only">{restaurant.name}: carta en vídeo</h1>
    {introVisible&&restaurant.logo_url&&<div data-menu-intro role="status" aria-label={`Abriendo la carta de ${restaurant.name}`} style={{background:colors.background}} className="fixed inset-0 z-[70] mx-auto grid max-w-[430px] place-items-center overflow-hidden p-8 text-center lg:max-w-none"><button type="button" aria-label="Abrir carta" onClick={()=>setIntroVisible(false)} className="grid h-44 w-full place-items-center"><span role="img" aria-label={`Logo de ${restaurant.name}`} style={{backgroundImage:`url(${restaurant.logo_url})`}} className="h-full w-full max-w-[280px] bg-contain bg-center bg-no-repeat"/></button></div>}
    <header style={{background:`linear-gradient(to bottom,${colors.background}f2,${colors.background}a8,transparent)`}} className="pointer-events-none fixed left-0 right-0 top-0 z-30 mx-auto flex max-w-[430px] items-center justify-between px-4 pb-10 pt-[max(1rem,env(safe-area-inset-top))] lg:max-w-[1040px] lg:px-8">
    ...
    <div aria-hidden="true" className="pointer-events-none fixed right-3 top-1/2 z-20 flex -translate-y-1/2 flex-col gap-1.5 md:right-[calc((100vw-430px)/2+12px)] lg:right-5">{products.map((product,index)=><span key={product.id} style={{background:index===active?colors.accent:"rgba(255,255,255,.4)"}} className={`w-1 rounded-full transition-all ${active===index?"h-6":"h-1"}`}/>)}</div>

    {panel&&<div className="fixed inset-0 z-50 mx-auto flex max-w-[430px] items-end bg-black/65 p-3 backdrop-blur-sm lg:max-w-none lg:items-center lg:justify-center lg:p-8" onClick={()=>setPanel(null)}>
      <aside aria-label={panel==="menu"?text.menu:panel==="cart"?text.cart:text.info} style={{background:colors.panel,borderColor:colors.frame}} className="flex max-h-[88vh] max-h-[88dvh] w-full flex-col rounded-xl border p-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] shadow-2xl lg:max-w-4xl lg:p-7" onClick={event=>event.stopPropagation()}>
        ...
        {panel==="menu"?<div className="mt-3 min-h-0 overflow-y-auto pr-1"><p className="mb-3 text-[11px] text-white/55">{text.listHint}</p><p aria-live="polite" className="sr-only">{catalogAdded?`${products.find(product=>product.id===catalogAdded)?.name??"Producto"} ${language==="es"?"añadido al carrito":"added to cart"}`:""}</p><div className="space-y-4 lg:space-y-6">{categoryGroups.map(group=><section key={group.id}><h3 style={{color:colors.accent}} className="mb-2 text-[10px] font-bold uppercase tracking-[.14em] lg:text-xs">{group.name}</h3><div className="grid grid-cols-2 gap-2 lg:grid-cols-4 lg:gap-4">{group.products.map(product=>{const added=catalogAdded===product.id;return <article key={product.id} className="min-w-0 overflow-hidden rounded-xl border border-white/10 bg-white/[.04]"><button type="button" aria-label={`Ver ${product.name}`} onClick={()=>go(`product-${product.id}`)} className="relative block aspect-[4/3] w-full overflow-hidden bg-black/30">{product.image_url?<span role="img" aria-label={product.name} style={{backgroundImage:`url(${product.image_url})`}} className="block h-full w-full bg-cover bg-center"/>:<span className="grid h-full place-items-center bg-gradient-to-br from-white/10 to-black/30 text-white/45"><List size={20}/></span>}<span className="absolute inset-0 bg-gradient-to-t from-black/45 to-transparent"/></button><div className="p-2 lg:p-3"><button type="button" onClick={()=>go(`product-${product.id}`)} className="line-clamp-1 w-full text-left text-xs font-semibold leading-tight lg:text-sm">{translatedField(product,"name",language,product.name)}</button><div className="mt-1.5 flex items-center justify-between gap-2"><strong style={{color:colors.accent}} className="text-xs tabular-nums lg:text-sm">{currency.format(product.price_cents/100)}</strong><button type="button" aria-label={added?`${product.name} añadido al carrito`:`${text.add} ${product.name}`} onClick={()=>addFromCatalog(product.id)} style={{background:colors.accent,color:colors.background}} className={`grid h-7 w-7 shrink-0 place-items-center rounded-full transition-transform duration-200 lg:h-9 lg:w-9 ${added?"scale-110":"active:scale-90"}`}>{added?<Check size={15} strokeWidth={3}/>:<Plus size={14}/>}</button></div></div></article>})}</div></section>)}</div></div>
    ...
    <div>{products.map((product,index)=>{const description=translatedField(product,"description",language,product.description);const allergens=(product.allergens??[]) as AllergenCode[];return <section ref={element=>{sectionRefs.current[index]=element}} data-index={index} id={`product-${product.id}`} key={product.id} className="relative isolate flex h-screen h-dvh snap-start snap-always items-end overflow-hidden bg-[var(--theme-bg)] px-4 pb-[calc(5.5rem+max(.75rem,env(safe-area-inset-bottom)))] pt-24 lg:items-center lg:justify-center lg:pb-16 lg:pl-[440px] lg:pr-24 lg:pt-24">
      <div style={{borderColor:colors.frame}} className={`absolute z-0 overflow-hidden bg-[#22221f] ${framed?"inset-3 bottom-16 rounded-xl border shadow-2xl":"inset-0"} lg:inset-y-20 lg:left-[calc(50%-430px)] lg:right-auto lg:w-[360px] lg:rounded-xl lg:border lg:shadow-2xl`}><ProductMedia index={index} name={product.name} src={product.video_url} poster={product.image_url} muted={muted} preload={Math.abs(index-active)<=1?"auto":"metadata"} active={index===active&&!introVisible} hydrated={Math.abs(index-active)<=1} reducedMotion={reducedMotion} playbackBlocked={playbackBlocked.has(index)} setVideoRef={element=>{videoRefs.current[index]=element}} onPlaybackStarted={playbackStarted}/></div>
      <div className={`absolute z-[1] ${framed?"inset-3 bottom-16 rounded-xl":"inset-0"} lg:inset-y-20 lg:left-[calc(50%-430px)] lg:right-auto lg:w-[360px] lg:rounded-xl`} style={{background:`linear-gradient(180deg,${colors.background}66 0%,transparent 32%,transparent 45%,${colors.background}f2 100%)`}}/>
      ...
      <div data-product-details className="relative z-10 w-full max-h-[calc(100dvh-11rem-env(safe-area-inset-top)-env(safe-area-inset-bottom))] overflow-y-auto overscroll-contain pb-0.5 text-shadow-lg [scrollbar-width:none] [&::-webkit-scrollbar]:hidden lg:max-h-[calc(100dvh-10rem)] lg:max-w-[430px] lg:px-4 lg:py-3">
        <div className="mb-1 flex min-w-0 items-center gap-2"><span style={{color:colors.accent}} className="max-w-[65%] truncate text-[9px] font-bold uppercase tracking-[.13em]">{translatedField(product.categories??{},"name",language,product.categories?.name)}</span>{product.is_featured&&<span className="truncate text-[9px] font-medium text-white/65">★ {text.featured}</span>}</div>
        <h2 className="line-clamp-1 max-w-[340px] text-[clamp(1.3rem,6vw,1.75rem)] font-semibold leading-tight tracking-[-.025em] lg:max-w-[430px] lg:text-4xl">{translatedField(product,"name",language,product.name)}</h2>
        <div className="mt-1.5 flex items-center justify-between gap-3 lg:mt-4"><strong style={{color:colors.accent}} className="text-lg font-semibold tabular-nums lg:text-2xl">{currency.format(product.price_cents/100)}</strong><button onClick={()=>addProduct(product.id)} style={{background:colors.accent,color:colors.background}} className="flex min-h-9 items-center gap-1.5 rounded-full px-3 text-xs font-bold shadow-lg transition active:scale-95 lg:min-h-11 lg:px-5 lg:text-sm"><Plus size={15}/>{text.add}</button></div>
        {(description||allergens.length>0)&&<details onToggle={event=>revealExpandedDetails(event.currentTarget)} className="group mt-1 lg:mt-3"><summary className="flex cursor-pointer list-none items-center gap-1 text-[11px] font-semibold text-white/75 lg:text-sm">{text.description}<ChevronDown size={13} className="transition-transform group-open:rotate-180"/></summary>{description&&<p className="mt-1 text-xs leading-[1.35] text-white/70 lg:mt-2 lg:text-sm lg:leading-relaxed">{description}</p>}{allergens.length>0&&<div className="mt-2 border-t border-white/10 pt-1.5"><p className="flex items-center gap-1 text-[10px] font-semibold text-white/65 lg:text-xs"><TriangleAlert size={11}/>{text.allergens} · {allergens.length}</p><div className="mt-1 flex flex-wrap gap-1">{allergens.map(code=><span key={code} style={{borderColor:colors.frame,background:`${colors.panel}d9`}} className="rounded-full border px-2 py-0.5 text-[9px] font-semibold lg:text-[10px]">{allergenLabel(code,language)}</span>)}</div><p className="mt-1 text-[9px] leading-snug text-white/50 lg:text-[10px]">{text.allergenNotice}</p></div>}</details>}
      </div>
    </section>})}</div>

    <nav ref={categoryNavRef} aria-label={text.categories} className="fixed left-1/2 top-[calc(max(1rem,env(safe-area-inset-top))+3.25rem)] z-40 flex w-[calc(100%-2rem)] max-w-[390px] -translate-x-1/2 snap-x snap-mandatory gap-2 overflow-x-auto [mask-image:linear-gradient(to_right,transparent,black_14%,black_86%,transparent)] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden lg:max-w-[720px]">
      <span aria-hidden="true" className="w-[calc((100%-1rem)/3)] shrink-0"/>
      {categoryGroups.map(group=>{const selected=activeCategory===group.id;return <button ref={element=>{if(element)categoryButtonRefs.current.set(group.id,element);else categoryButtonRefs.current.delete(group.id)}} key={group.id} type="button" aria-current={selected?"true":undefined} onClick={()=>{const index=products.findIndex(product=>product.id===group.products[0].id);if(index>=0)setActive(index);go(`product-${group.products[0].id}`,true)}} style={selected?{background:colors.accent,color:colors.background,borderColor:colors.accent}:{background:`${colors.nav}ed`,borderColor:colors.frame}} className={`w-[calc((100%-1rem)/3)] shrink-0 snap-center truncate rounded-full border px-2 py-1.5 text-[10px] font-bold shadow-lg backdrop-blur-xl transition-opacity duration-300 lg:w-40 lg:text-xs ${selected?"opacity-100":"opacity-[.45]"}`}>{group.name}</button>})}
      <span aria-hidden="true" className="w-[calc((100%-1rem)/3)] shrink-0"/>
    </nav>

    <nav aria-label="Controles de la carta" style={{background:`${colors.nav}ed`,borderColor:colors.frame}} className="fixed bottom-[max(.75rem,env(safe-area-inset-bottom))] left-1/2 z-40 grid w-[calc(100%-2rem)] max-w-[390px] -translate-x-1/2 grid-cols-5 items-center rounded-xl border px-1 py-1 shadow-2xl backdrop-blur-xl lg:bottom-1/2 lg:left-auto lg:right-[max(1.5rem,calc(50%-520px))] lg:w-14 lg:max-w-none lg:translate-x-0 lg:translate-y-1/2 lg:grid-cols-1 lg:gap-2 lg:py-2">
