...
export function BackButton({fallback="/dashboard"}:{fallback?:string}){
  ...
  return <button type="button" onClick={goBack} className="focus inline-flex min-h-11 items-center gap-2 rounded-xl border border-white/25 bg-white/[.04] px-3.5 py-2 text-sm font-semibold transition-colors hover:bg-white/10 active:bg-white/15" aria-label="Volver a la sección anterior del panel"><ArrowLeft size={18}/>Volver</button>;
}
