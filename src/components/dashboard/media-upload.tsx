...
export function MediaUpload({restaurantId,kind="logo",products=[],label,currentUrl}:{restaurantId:string;kind?:Kind;products?:Option[];label?:string;currentUrl?:string|null}){
  ...
  function choose(candidate:File){
    ...
    const validExtension=video?["mp4","webm","mov"].includes(extension??""):["jpg","jpeg","png","webp"].includes(extension??"");
    if(!valid.includes(candidate.type)&&!validExtension){toast.error("Formato de archivo no válido.");return}
  ...
  return <div className="glass rounded-xl p-4">
    ...
    {uploading&&video&&<div className="mt-3"><div className="mb-1 flex justify-between text-xs font-medium text-slate-600"><span>Subida segura reanudable</span><span>{progress}%</span></div><div className="h-2 overflow-hidden rounded-full bg-stone-200"><div className="h-full bg-orange-600 transition-[width]" style={{width:`${progress}%`}}/></div></div>}
    {file&&<div className="mt-3 flex gap-2"><button type="button" disabled={uploading} onClick={()=>void upload()} className="rounded-lg bg-orange-600 px-4 py-2 font-semibold text-white disabled:opacity-50">{uploading?(video?`Subiendo ${progress}%`:"Subiendo…"):"Confirmar subida"}</button><button type="button" disabled={uploading} onClick={()=>{setFile(null);setVideoInfo(null);setProgress(0)}} className="inline-flex items-center gap-1 rounded-lg border border-stone-300 px-3 py-2"><X size={16}/>Cancelar</button></div>}
  </div>;
}
