...
export function AppearancePreferences({enabled,template,canUsePremium,restaurantName,logoUrl,currency,previewProduct}:{enabled:boolean;template?:string;canUsePremium:boolean;restaurantName:string;logoUrl:string|null;currency:string;previewProduct?:PreviewProduct}){
  ...
  return <>
    <form action={async form=>{try{const result=await updateAppearancePreferences(form);toast.success("Preferencias guardadas");notifyAutomaticTranslation(result.translationStatus)}catch(error){toast.error(error instanceof Error?error.message:"No se pudo guardar")}}} className="space-y-6 rounded-2xl border border-stone-200 bg-white shadow-sm p-5">
      <section>
        <div className="flex items-end justify-between gap-4"><div><h2 className="font-bold">Plantillas de la carta</h2><p className="mt-1 text-sm text-slate-600">Previsualiza cada estilo antes de elegirlo.</p></div><span className="text-xs text-slate-500">{Object.keys(MENU_TEMPLATES).length} disponibles</span></div>
        ...
        {!canUsePremium&&<div className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-amber-400/15 bg-amber-400/[.06] p-3 text-xs text-slate-600"><span>Puedes previsualizar las plantillas premium antes de activarlas.</span><Link href="/dashboard/billing?from=templates" className="font-semibold text-amber-300 hover:text-amber-200">Ver Plan Carta →</Link></div>}
      </section>
