...
function TemplatePreview({kind,restaurantName,logoUrl,currency,product,large=false}:PreviewProps){
  ...
  if(kind==="noirluxe")return <div style={{fontFamily:"var(--font-noir-sans)"}} className={`relative isolate mx-auto w-full overflow-hidden bg-[#111111] text-[#F0E9DB] shadow-2xl ${large?"h-[min(70dvh,620px)] max-w-[350px]":"aspect-[9/12]"}`}>
    ...
    <header className={`absolute inset-x-0 top-0 z-10 flex items-start justify-between bg-gradient-to-b from-[#111111]/40 to-transparent ${large?"px-6 pb-12 pt-8":"px-3 pb-6 pt-3"}`}><NoirLuxeHamburgerIcon/><div className="min-w-0 flex-1 px-2 text-center">{logoUrl&&<span role="img" aria-label={`Logo de ${restaurantName}`} className={`mx-auto block bg-contain bg-center bg-no-repeat ${large?"h-8 w-28":"h-5 w-16"}`} style={{backgroundImage:`url(${logoUrl})`}}/>}</div><span className={large?"scale-100 origin-top-right":"scale-50 origin-top-right"}><NoirLuxeBasketIcon/></span></header>
    <nav aria-label="Vista de categorías" className={`absolute inset-x-0 z-10 flex overflow-hidden uppercase ${large?`top-24 gap-7 px-6 ${NOIRLUXE_TOKENS.typography.category}`:"top-12 gap-3 px-3 text-[6px] leading-[8px] tracking-[.16em]"}`}><span className="shrink-0 border-b border-[#C9A96E] pb-1 text-white">{product?.category??"Entrantes"}</span><span className="shrink-0 border-b border-[#111111] text-[#F0E9DB]">Principales</span><span className="shrink-0 border-b border-[#111111] text-[#F0E9DB]">Postres</span></nav>
    <div className={`absolute inset-x-0 bottom-0 z-10 ${large?"p-6":"p-3"}`}><div className="flex items-center justify-between gap-2"><span className={`bg-[#111111]/40 uppercase text-white ${large?`px-2 py-1 ${NOIRLUXE_TOKENS.typography.badge}`:"px-1.5 py-1 text-[5px] leading-[8px]"}`}>{product?.category??"Especialidades"}</span><span className={large?"scale-100":"scale-50 origin-right"}><NoirLuxeAddIcon/></span></div><p className={`text-[#C9A96E] ${large?`mt-4 ${NOIRLUXE_TOKENS.typography.dishName}`:"mt-2 font-[var(--font-noir-serif)] text-base font-normal italic leading-[18px]"}`}>{product?.name??"Producto destacado"}</p><p className={`text-[#F0E9DB] ${large?`mt-2 ${NOIRLUXE_TOKENS.typography.body}`:"mt-1 text-[6px] font-light leading-3"}`}>Una propuesta especial de nuestra cocina.</p><div className={`flex items-center justify-between ${large?"mt-5":"mt-2"}`}><strong className={`text-[#C9A96E] ${large?NOIRLUXE_TOKENS.typography.price:"font-[var(--font-noir-serif)] text-sm font-normal leading-4"}`}>{price}</strong><span className={large?"scale-100 origin-right":"scale-50 origin-right"}><NoirLuxeProgress active={0} total={3}/></span></div></div>
  </div>;
