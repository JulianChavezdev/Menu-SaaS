...
type Restaurant={opening_hours?:import("./order-settings").OrderSettings["opening_hours"];customer_order_mode?:"menu_only"|"table_orders";payment_timing?:"before"|"after";customer_orders_paused?:boolean;id:string;name:string;slug:string;is_published:boolean;subscription_status:"trialing"|"active"|"past_due"|"canceled";signup_plan_interest?:"carta"|"pedidos"|"configuracion";ordering_enabled?:boolean;pickup_enabled?:boolean;pickup_paused?:boolean;access_suspended?:boolean;suspension_reason?:string|null;suspended_at?:string|null;language_switcher_enabled?:boolean;menu_template?:string;primary_color:string;secondary_color:string;description:string|null;translations?:Record<string,{name?:string;description?:string}>|null;phone:string|null;email:string|null;address:string|null;instagram_url:string|null;website_url:string|null;logo_url:string|null;cover_url:string|null;currency:string;locale:string;timezone:string};
...
export async function canAddProduct(restaurantId:string,categoryId:string){const supabase=await createClient();const[{count},{count:categoryCount},{data:restaurant}]=await Promise.all([supabase.from("products").select("id",{count:"exact",head:true}).eq("restaurant_id",restaurantId),supabase.from("products").select("id",{count:"exact",head:true}).eq("restaurant_id",restaurantId).eq("category_id",categoryId),supabase.from("restaurants").select("subscription_status").eq("id",restaurantId).single()]);return canCreateProduct(count??0,planForStatus(restaurant?.subscription_status??"past_due"),categoryCount??0)}
