...
function endpoint(key:string){return key.endsWith(":fx")?"https://api-free.deepl.com/v2/translate":"https://api.deepl.com/v2/translate"}
...
export async function translateTextsToEnglish(texts:string[],options:{apiKey?:string;fetcher?:Fetcher}={}):Promise<AutomaticTranslationResult<string[]>>{
  ...
  try{
    for(let start=0;start<clean.length;start+=40){const batch=clean.slice(start,start+40);const nonEmpty=batch.map((text,index)=>({text,index})).filter(item=>item.text);if(!nonEmpty.length){translated.push(...batch);continue}
      const response=await fetcher(endpoint(key),{method:"POST",headers:{Authorization:`DeepL-Auth-Key ${key}`,"Content-Type":"application/json"},body:JSON.stringify({text:nonEmpty.map(item=>item.text),source_lang:"ES",target_lang:"EN-US"}),cache:"no-store",signal:AbortSignal.timeout(12_000)});
