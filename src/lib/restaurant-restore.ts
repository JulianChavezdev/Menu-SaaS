...
const productSchema=z.object({
  ...
  image_url:optionalText(2048),image_path:optionalText(1024),is_available:z.boolean(),is_featured:z.boolean(),
...
export function buildRestorePreview(backup:RestaurantRestoreBackup,current:CurrentRestaurant,currentCategoryIds:string[],currentProductIds:string[]){
  ...
  const exceedsPlan=current.subscription_status!=="active"&&(backup.categories.length>5||backup.products.length>3);
  return{
