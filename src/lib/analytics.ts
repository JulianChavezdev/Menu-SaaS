...
export const analyticsEventSchema=z.object({restaurantId:z.string().uuid(),productId:z.string().uuid().nullable().optional(),event:z.enum(["menu_view","product_view","video_play","cart_add","share","contact_click"]),locale:z.enum(["es","en"]).default("es")}).superRefine((value,context)=>{if(["product_view","video_play","cart_add"].includes(value.event)&&!value.productId)context.addIssue({code:"custom",path:["productId"],message:"Product events require a product"})});
