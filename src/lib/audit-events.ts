...
export function describeAuditEvent(action:string,rawDetails:unknown):Description{
  ...
  switch(action){
    ...
    case"platform.trash_cleanup_completed":return{title:"Tareas automáticas completadas",description:`${number(details,"trialsExpired")??0} pruebas vencidas, ${number(details,"processed")??0} copias revisadas y ${number(details,"failed")??0} fallos.`,group:"system",tone:"success"};
