...
export const metadata:Metadata={title:"Aviso legal | Carta Video",robots:{index:false,follow:false}};

export default function LegalNoticePage(){const identity=getLegalIdentity();if(!identity.complete)notFound();return <LegalPage title="Aviso legal" summary="Información sobre la persona responsable de Carta Video y las reglas generales de acceso al sitio." identity={identity}>
  <LegalSection title="1. Titular del sitio"><p>Este sitio y el servicio Carta Video son ofrecidos por {identity.name}, con NIF/CIF {identity.taxId} y domicilio en {identity.address}. Puedes contactar en {identity.email}{identity.phone?` o en ${identity.phone}`:""}.{identity.registry&&<> Datos registrales: {identity.registry}.</>}</p></LegalSection>
  <LegalSection title="2. Finalidad"><p>Carta Video es una plataforma dirigida principalmente a restaurantes y profesionales de hostelería para crear, gestionar y publicar cartas digitales con contenido audiovisual.</p></LegalSection>
