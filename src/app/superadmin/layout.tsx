...
export default async function SuperadminLayout({children}:{children:React.ReactNode}){
  const{admin}=await requireSuperadmin();const since=new Date(Date.now()-7*86_400_000).toISOString();const[{count:feedbackCount},{count:alertCount}]=await Promise.all([admin.from("restaurant_feedback").select("id",{count:"exact",head:true}).eq("status","new"),admin.from("superadmin_audit_log").select("id",{count:"exact",head:true}).like("action","alert.%").gte("created_at",since)]);
