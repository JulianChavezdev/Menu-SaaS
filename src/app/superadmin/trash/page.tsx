...
export default async function RestaurantTrashPage(){
  ...
  return <main className="mx-auto max-w-5xl p-4 md:p-6"><Link href="/superadmin" className="text-sm text-violet-300 hover:text-violet-200">← Volver a restaurantes</Link><div className="mt-5 flex items-start gap-3"><span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-red-500/10 text-red-300"><Trash2 size={23}/></span><div><p className="text-xs font-bold uppercase tracking-[.18em] text-red-300">Recuperación segura</p><h1 className="mt-1 text-3xl font-extrabold">Papelera de restaurantes</h1><p className="mt-2 max-w-2xl text-sm leading-relaxed text-slate-400">Puedes recuperar un restaurante durante {RESTAURANT_RESTORE_DAYS} días. Se restaurará suspendido y sin publicar para que revises sus datos antes de reactivarlo. Una limpieza automática diaria elimina las copias y medios cuando vence el plazo.</p></div></div>
    <section className="mt-5 rounded-2xl border border-white/10 bg-slate-950/60 p-4" aria-label="Estado de limpieza automática">{lastCleanup?<div className="flex items-start gap-3">{cleanupSucceeded?<CheckCircle2 className="mt-0.5 shrink-0 text-emerald-400" size={20}/>:<TriangleAlert className="mt-0.5 shrink-0 text-red-400" size={20}/>}<div><p className="text-sm font-bold">{cleanupSucceeded?"Última limpieza completada":"La última limpieza falló"}</p><p className="mt-1 text-xs leading-relaxed text-slate-400">{new Intl.DateTimeFormat("es-ES",{dateStyle:"medium",timeStyle:"short"}).format(new Date(lastCleanup.created_at))}{cleanupSucceeded&&<> · {Number(cleanupDetails.processed??0)} revisados · {Number(cleanupDetails.purged??0)} eliminados · {Number(cleanupDetails.restored??0)} restaurados · {Number(cleanupDetails.failed??0)} fallidos</>}</p></div></div>:<div className="flex items-start gap-3"><Clock3 className="mt-0.5 shrink-0 text-slate-400" size={20}/><div><p className="text-sm font-bold">Primera limpieza pendiente</p><p className="mt-1 text-xs text-slate-400">Vercel la ejecutará diariamente a las 03:15 UTC.</p></div></div>}</section>
