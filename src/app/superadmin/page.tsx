...
export default async function SuperadminPage({searchParams}:{searchParams:Promise<{expiration?:string;processed?:string;cleanup?:string}>}){
  ...
  const[{data,error},{data:analytics,error:analyticsError},{data:resourceRows,error:resourceError},{data:cleanupRuns,error:cleanupError}]=await Promise.all([admin.from("restaurants").select("id,name,slug,is_published,access_suspended,subscription_status,menu_template,created_at,products(count),categories(count),restaurant_members(count),subscriptions(provider,current_period_end)").order("created_at",{ascending:false}),admin.from("menu_analytics_daily").select("event_type,event_count,product_id,products(video_path)").gte("event_date",analyticsFrom.toISOString().slice(0,10)),admin.rpc("get_platform_resource_metrics"),admin.from("superadmin_audit_log").select("action,details,created_at").in("action",["platform.trash_cleanup_completed","platform.trash_cleanup_failed"]).order("created_at",{ascending:false}).limit(1)]);
