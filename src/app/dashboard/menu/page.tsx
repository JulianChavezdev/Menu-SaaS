...
export default async function Page(){
  ...
  return <main className="mx-auto max-w-7xl p-4 md:p-6">
    ...
    <details id="categorias" className="group mt-5 rounded-3xl border border-slate-800/80 bg-slate-950/40 p-4 md:p-5">
      <summary className="flex min-h-11 cursor-pointer list-none items-center justify-between gap-3 font-semibold"><span>Categorías <span className="ml-2 text-sm font-normal text-slate-400">{categories?.length??0}</span></span><span className="text-sm text-violet-300 group-open:hidden">Gestionar</span><span className="hidden text-sm text-violet-300 group-open:inline">Cerrar</span></summary>
      <div className="mt-5 border-t border-slate-800 pt-5"><CategoriesManager categories={categories??[]}/></div>
    </details>
    <div className="mt-5 grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
      <section className="min-w-0 rounded-3xl border border-slate-800/80 bg-slate-950/40 p-4 md:p-5"><h2 className="mb-4 text-lg font-bold">Productos</h2><ProductsManager categories={categories??[]} products={list}/></section>
