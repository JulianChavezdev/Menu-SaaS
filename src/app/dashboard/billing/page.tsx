...
export default async function Page({searchParams}:{searchParams:Promise<{from?:string;checkout?:string}>}){
  ...
  return <main className="mx-auto max-w-5xl p-4 md:p-6">
    ...
    {!active&&<div className="mt-5 border border-amber-300 bg-amber-50 p-4 text-sm text-amber-950"><strong>{status==="trialing"?`Periodo de prueba: ${trialDays??7} día${trialDays===1?"":"s"} restante${trialDays===1?"":"s"}.`:"Pago necesario para publicar la carta."}</strong><span className="mt-1 block text-amber-800">La prueba dura 7 días. Al terminar, la carta pública se suspende hasta que se registre un pago.</span></div>}
