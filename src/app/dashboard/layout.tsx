...
export default async function DashboardLayout({children}:{children:React.ReactNode}){
  ...
  return <div className="dashboard-light min-h-screen bg-[#f4f1eb] text-slate-950 md:grid md:grid-cols-[240px_1fr]">
    <aside className="border-b border-stone-200 bg-white p-4 shadow-sm md:sticky md:top-0 md:flex md:h-screen md:flex-col md:border-b-0 md:border-r">
      <div className="flex items-center justify-between gap-3 md:block"><Link href="/dashboard" prefetch className="inline-flex items-center gap-2 text-lg font-black tracking-tight"><span className="h-5 w-2 bg-orange-600"/>Carta Video</Link><div className="md:mt-5"><RestaurantSwitcher activeId={restaurant.id} items={items}/></div></div>
