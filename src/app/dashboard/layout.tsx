...
export default async function DashboardLayout({children}:{children:React.ReactNode}){
  ...
  return <div className="min-h-screen md:grid md:grid-cols-[240px_1fr]">
    <aside className="border-b border-white/10 bg-slate-950/95 p-4 md:sticky md:top-0 md:flex md:h-screen md:flex-col md:border-b-0 md:border-r">
      <div className="flex items-center justify-between gap-3 md:block"><Link href="/dashboard" prefetch className="text-lg font-bold">Carta Video</Link><div className="md:mt-5"><RestaurantSwitcher activeId={restaurant.id} items={items}/></div></div>
      <DashboardNavigation links={navigation}/>
      <div className="mt-8 border-t border-white/10 pt-4 md:mt-auto"><SignOut/></div>
    </aside>
