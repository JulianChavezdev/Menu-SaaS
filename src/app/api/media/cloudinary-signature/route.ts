...
export async function POST(request:Request){
  ...
  try{if(origin&&new URL(origin).origin!==new URL(request.url).origin)return NextResponse.json({error:"Origen no válido"},{status:403,headers})}
  ...
  const{data:member}=await supabase.from("restaurant_members").select("role,restaurants(access_suspended,subscription_status)").eq("restaurant_id",parsed.data.restaurantId).eq("user_id",user.id).maybeSingle();
  const restaurant=member?.restaurants as unknown as {access_suspended?:boolean;subscription_status:string}|null;if(!member||!canUploadRestaurantMedia(member.role,restaurant))return NextResponse.json({error:"Sin permiso"},{status:403,headers});
