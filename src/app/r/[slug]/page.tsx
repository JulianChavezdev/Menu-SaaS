...
export async function generateMetadata({
  ...
  if (slug === "bistro-nube" && !process.env.NEXT_PUBLIC_SUPABASE_URL)
    return {
      ...
      description: demoRestaurant.description ?? undefined,
  ...
  return {
    ...
    alternates: { canonical: `/r/${resolved.slug}` },
    openGraph: {
      title: resolved.name,
      description: resolved.description ?? "Carta digital en vídeo",
      images: resolved.logo_url ? [resolved.logo_url] : undefined,
    },
  };
}
...
export default async function PublicMenu({
  ...
  const supabase = await menuDatabase();
  const { data: restaurant } = await retryPublicQuery(() =>
    supabase
      .from("restaurants")
      ...
      <main className="grid min-h-screen place-items-center bg-slate-950 p-6 text-center">
        <div className="glass max-w-md rounded-2xl p-6">
          ...
          <p className="mt-3 text-slate-300">
            La carta todavía no tiene productos disponibles.
          </p>
        </div>
      </main>
    );
  const { data: recommendations } = await supabase
    ...
    .select("source_product_id,recommended_product_id,sort_order")
    .eq("restaurant_id", restaurant.id)
    .order("sort_order");
  ...
  const productsWithRecommendations = products.map((product) => ({
    ...
    recommended_products: (recommendations ?? [])
      .filter((item) => item.source_product_id === product.id)
      .flatMap((item) => {
        const recommended = productsById.get(item.recommended_product_id);
        return recommended
          ? [
              {
                id: recommended.id,
                name: recommended.name,
                price_cents: recommended.price_cents,
                image_url: recommended.image_url,
                is_available: recommended.is_available,
                translations: recommended.translations,
              },
            ]
          : [];
      }),
