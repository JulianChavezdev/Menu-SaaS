...
const ready={subscriptionStatus:"active",trialDays:null,published:true,products:3,productsWithoutMedia:0,menuViews:20,cartAdds:4,recommendationAdds:1,hasLogo:true,hasContact:true};
describe("restaurant action center",()=>{
  ...
  it("limits the dashboard to six ordered alerts",()=>expect(restaurantAlerts({subscriptionStatus:"canceled",trialDays:0,published:false,products:2,productsWithoutMedia:2,menuViews:0,cartAdds:0,recommendationAdds:0,hasLogo:false,hasContact:false}).length).toBeLessThanOrEqual(6));
});
