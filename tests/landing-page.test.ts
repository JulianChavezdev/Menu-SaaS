...
describe("landing pública",()=>{
  ...
  it("publica los dos planes sin prueba gratuita",()=>{for(const copy of["Plan Carta","34,99 €","344,30 €/año","ahorra un 18%","Configuración completa","149,99 €","Segundo mes gratis","Edición de vídeos con IA","Primer mes de Plan Carta incluido"])expect(page).toContain(copy);expect(page).not.toContain('name="Prueba"')});
  ...
  it("enlaza el manual para restaurantes",()=>{expect(page).toContain('/manual-menuly-restaurantes.pdf');expect(page).toContain('Manual')});
