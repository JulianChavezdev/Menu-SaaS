...
describe("virtualización del feed de vídeo",()=>{
  ...
  it("precarga durante la apertura de marca sin adelantar el primer vídeo",()=>{expect(menu).toContain("if(introVisible){video.pause();video.currentTime=0;return}");expect(menu).toContain("active={index===active&&!introVisible}");expect(menu).toContain("if(introVisible)return")});
