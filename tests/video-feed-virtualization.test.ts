...
describe("virtualización del feed de vídeo",()=>{
  ...
  it("publica una señal estable y renderiza los primeros vídeos desde el HTML",()=>{expect(menu).toContain('data-hydrated={hydrated?"true":"false"}');expect(menu).toContain("const hydrated=true");expect(menu).not.toContain("setHydrated")});
  ...
  it("deja la plantilla principal NoirLuxe sin vectores decorativos",()=>{expect(menu).toContain('template.key==="noirluxe"');expect(vectors).toContain('if(motif==="noirluxe")return null')});
  it("mueve los controles de NoirLuxe al menú superior y conserva el carrito separado",()=>{expect(menu).toContain('onClick={()=>setPanel("controls")}');expect(menu).toContain('panel==="controls"');expect(menu).toContain('onClick={()=>setPanel("menu")}');expect(menu).toContain('onClick={()=>setPanel("cart")}');expect(menu).toContain('fontFamily:noirLuxe?"var(--font-noir-sans)"');expect(menu).toContain('[&_h2]:[font-family:var(--font-noir-serif)]')});
});
