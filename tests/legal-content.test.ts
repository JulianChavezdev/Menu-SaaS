...
describe("legal publication safeguards", () => {
  it("requires the mandatory identity fields", () => {
    ...
    expect(source).toContain(
      "identity.name && identity.taxId && identity.address && identity.email",
    );
  });

  it.each(["legal", "privacidad", "cookies", "condiciones", "encargo-datos"])(
    "protects /%s until identity is complete",
    (route) => {
  ...
  it("distinguishes local carts, kitchen orders and fiscal POS functions", () => {
    ...
    expect(terms).toContain("Menuly Comandas");
    const normalizedTerms = terms.replace(/\s+/g, " ");
    const normalizedPrivacy = privacy.replace(/\s+/g, " ");
    expect(normalizedTerms).toContain("no sustituye un TPV fiscal");
    expect(normalizedPrivacy).toContain("personal se transmiten");
    expect(normalizedPrivacy).toContain("al enviar la comanda");
  });

  it("makes the restaurant responsible for allergen review", () => {
    const terms = read("src/app/condiciones/page.tsx");
    expect(terms.replace(/\s+/g, " ")).toContain(
