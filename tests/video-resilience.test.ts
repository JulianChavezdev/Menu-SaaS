...
describe("public video resilience",()=>{
  ...
  it("keeps the poster and recovers stalls without a blocking reload prompt",()=>{expect(media).toContain('status==="loading"');expect(media).toContain('status==="error"');expect(media).not.toContain("Reanudarvídeo");expect(media).toContain("Reintentar");expect(media).toContain("onStalled");expect(media).toContain("poster")});
  ...
  it("recovers when Safari loads media before React attaches events",()=>{expect(media).toContain("video.readyState>=HTMLMediaElement.HAVE_CURRENT_DATA");expect(media).toContain('status==="error"?"opacity-0":"opacity-100"');expect(menu).toContain("onPointerUp={handlePointerUp}")});
});
