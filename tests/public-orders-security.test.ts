...
describe("public table orders",()=>{
  ...
  it("never trusts client prices or restaurant identity",()=>{expect(route).toContain('from("products")');expect(route).toContain("priceOrderLines(parsed.data.lines,products,restaurant.id)");expect(route).not.toContain("body.subtotal");expect(route).not.toContain("body.restaurantId")});
  ...
  it("creates the order and its items atomically",()=>{expect(route).toContain('rpc("create_public_dining_order"');expect(route).not.toContain('from("dining_order_items").insert')});
