...
describe("resumable video uploads",()=>{
  ...
  it("shows upload progress and mobile playback recommendations",()=>{expect(component).toContain("Subida segura reanudable");expect(component).toMatch(/onProgress:\s*setProgress/);expect(component).toContain("MP4 vertical, H.264");expect(component).toMatch(/videoInfo\.width\s*>\s*videoInfo\.height/)});
});
