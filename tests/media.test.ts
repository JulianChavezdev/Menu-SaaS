...
describe("media paths",()=>{
  it("accepts tenant-scoped logos, product photos and videos",()=>{
    ...
    expect(isValidMediaPath({kind:"product-image",restaurantId,productId,path:`restaurants/${restaurantId}/products/${productId}/image-${fileId}.webp`})).toBe(true);
  });
  ...
  it("rejects other tenants, unsupported types and mismatched products",()=>{
    ...
    expect(isValidMediaPath({kind:"product-image",restaurantId,productId,path:`restaurants/${restaurantId}/products/${crypto.randomUUID()}/image-${fileId}.jpg`})).toBe(false);
  });
