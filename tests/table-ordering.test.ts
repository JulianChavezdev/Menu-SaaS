...
describe("table ordering",()=>{
  it("accepts a bounded order",()=>expect(publicOrderSchema.safeParse({tableCode:"00000000-0000-4000-8000-000000000001",lines:[{productId:"00000000-0000-4000-8000-000000000002",quantity:2,note:"Sin cebolla"}],customerNote:""}).success).toBe(true));
  it("rejects duplicated products and excessive quantities",()=>expect(publicOrderSchema.safeParse({tableCode:"00000000-0000-4000-8000-000000000001",lines:[{productId:"00000000-0000-4000-8000-000000000002",quantity:21,note:""},{productId:"00000000-0000-4000-8000-000000000002",quantity:1,note:""}],customerNote:""}).success).toBe(false));
  it("only permits the kitchen workflow",()=>{expect(canTransitionOrder("pending","accepted")).toBe(true);expect(canTransitionOrder("pending","ready")).toBe(false);expect(canTransitionOrder("delivered","pending")).toBe(false)});
