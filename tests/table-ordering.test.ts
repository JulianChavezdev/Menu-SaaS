...
describe("table ordering",()=>{
  ...
  it("uses the simplified kitchen workflow",()=>{expect(canTransitionOrder("pending","ready")).toBe(true);expect(canTransitionOrder("pending","accepted")).toBe(false);expect(canTransitionOrder("rejected","pending")).toBe(true);expect(canTransitionOrder("delivered","pending")).toBe(false)});
