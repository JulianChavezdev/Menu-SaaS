...
describe("flujo de traducción automática",()=>{
  it("traduce al guardar todos los tipos de contenido",()=>{expect(actions.match(/translateFieldsToEnglish/g)?.length).toBeGreaterThanOrEqual(4);expect(actions).toContain("translateEntireMenu");expect(actions).toMatch(/mergeTranslation\(item\.translations,\s*"en"/)});
