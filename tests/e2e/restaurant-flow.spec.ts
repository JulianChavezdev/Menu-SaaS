...
test.describe("restaurant owner journey", () => {
  ...
  test.beforeAll(async () => {
    const { data, error } = await admin!.auth.admin.createUser({
      ...
      password,
      email_confirm: true,
      user_metadata: { plan_interest: "pedidos" },
    });
  ...
  test.afterAll(async () => {
    ...
    for (const restaurant of restaurants ?? []) {
      ...
      const { data: logos } = await admin.storage
      ...
      if (paths.length)
        await admin.storage.from("restaurant-media").remove(paths);
      await admin.from("restaurants").delete().eq("id", restaurant.id);
    }
    await admin.auth.admin.deleteUser(userId);
  });

  test("preselects the requested plan on the public registration form", async ({
    page,
  }) => {
    await page.goto("/register?plan=pedidos");
    await expect(
      page.getByRole("radio", { name: /Menuly Comandas/ }),
    ).toBeChecked();
    await expect(page.locator('input[name="legal_acceptance"]')).toHaveCount(0);
    await expect(
      page.locator('a[href="/condiciones"],a[href="/privacidad"]'),
    ).toHaveCount(0);
  });
  ...
  test("login, manual activation, guide, product creation and public menu", async ({
    ...
    await expect(
      page.getByText("Menuly Comandas", { exact: true }),
    ).toBeVisible();
    ...
    const created = await admin!
      .from("restaurants")
      .select(
        "id,subscription_status,signup_plan_interest,ordering_enabled,email",
      )
      .eq("slug", slug)
      .single();
    ...
    if (
      !(await categoriesDetails.evaluate(
        (element) => (element as HTMLDetailsElement).open,
      ))
    )
      await categoriesDetails.locator(":scope > summary").click();
    ...
    await expect(
      page.getByText("Entrantes", { exact: true }).first(),
    ).toBeVisible();
    ...
    await productForm
      ...
      .fill("Producto creado mediante el flujo completo.");
    await expect(
      productForm.getByText(
        "La versión inglesa se genera automáticamente al guardar.",
      ),
    ).toBeVisible();
    await productForm.getByLabel("Precio (€)").fill("9.50");
    await productForm
      .getByLabel("Categoría")
      .selectOption({ label: "Entrantes" });
    await productForm.getByRole("button", { name: "Crear producto" }).click();
    await expect(
      page.getByRole("heading", { name: "Producto E2E", exact: true }),
    ).toBeVisible();
    ...
    await expect(
      page.getByRole("button", { name: "Confirmar subida" }),
    ).toBeVisible();
    ...
    await expect(
      page.getByRole("heading", { name: "Producto E2E" }),
    ).toBeVisible();
    await expect(page.getByText("9,50 €")).toBeVisible();
    await expect(
      page.getByRole("img", { name: "Logo de Restaurante E2E" }).first(),
    ).toBeVisible();
    await expect(
      page.getByRole("navigation", { name: "Controles de la carta" }),
    ).toBeVisible();
    const detailsBox = await page
      .locator("[data-product-details]")
      .boundingBox();
    expect(detailsBox?.height ?? 844).toBeLessThan(844 * 0.35);
    ...
    await expect(
      page.getByText("Producto creado mediante el flujo completo."),
    ).toBeVisible();
