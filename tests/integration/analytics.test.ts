...
const url=process.env.NEXT_PUBLIC_SUPABASE_URL;const publicKey=process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY??process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;const serviceKey=process.env.SUPABASE_SECRET_KEY??process.env.SUPABASE_SERVICE_ROLE_KEY;
...
suite("privacy-safe menu analytics",()=>{
  it("aggregates published events and isolates restaurant statistics",async context=>{
    ...
    try{
      ...
      for(const result of [await record("menu_view"),await record("menu_view"),await record("product_view",product.data.id),await record("video_play",product.data.id),await record("cart_add",product.data.id),await record("share",null,"en")])if(result.error)throw result.error;
      ...
      const visible=await owner.from("menu_analytics_daily").select("event_type,event_count,locale").eq("restaurant_id",restaurantId);expect(visible.error).toBeNull();expect(visible.data?.find(row=>row.event_type==="menu_view")?.event_count).toBe(2);expect(visible.data?.find(row=>row.event_type==="video_play")?.event_count).toBe(1);expect(visible.data?.find(row=>row.event_type==="cart_add")?.event_count).toBe(1);expect(visible.data?.find(row=>row.event_type==="share")?.locale).toBe("en");
