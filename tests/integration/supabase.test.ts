...
suite("Supabase public isolation",()=>{
  ...
  it("exposes its complete consolidated menu",async()=>{const {data,error}=await supabase.from("products").select("name").eq("restaurant_id",bistroId);expect(error).toBeNull();expect(data).toHaveLength(22)});
