...
const salesGrowth=readFileSync("supabase/migrations/202607170002_sales_growth.sql","utf8");
...
describe("analytics migration",()=>{
  ...
  it("adds detail and attributed recommendation counters without visitor tracking",()=>{expect(salesGrowth).toContain("'detail_open'");expect(salesGrowth).toContain("'recommendation_add'");const executable=salesGrowth.split("comment on table")[0];for(const forbidden of["ip_address","user_agent","visitor_id","cookie"])expect(executable).not.toContain(forbidden)});
});
