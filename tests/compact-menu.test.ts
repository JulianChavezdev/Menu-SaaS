...
const preview = readFileSync(
  "src/components/dashboard/appearance-preferences.tsx",
  "utf8",
);
...
describe("ficha compacta de producto", () => {
  it("limita la altura y el texto en la carta pública", () => {
    ...
    expect(menu).toContain(
      "max-h-[calc(100dvh-11rem-env(safe-area-inset-top)-env(safe-area-inset-bottom))]",
    );
    ...
    expect(menu).toContain("pb-[var(--controls-clearance)]");
    expect(menu).toContain("controlsRef");
    expect(menu).toContain("feedBox.bottom - controlsBox.top + 16");
    expect(menu).not.toContain(
      "bottom-[calc(max(.75rem,env(safe-area-inset-bottom))+4.75rem)]",
    );
    ...
    expect(menu).toContain("text-[clamp(1.3rem,6vw,1.6rem)]");
    expect(menu).toContain("flex min-h-8 gap-2");
